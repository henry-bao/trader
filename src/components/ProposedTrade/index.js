  ...
  return (
    <div className="ptWrapper" ref={(node) => (wrapper = node)}>
      <div className="upper">
        ...
        {reqStatus === "PENDING" ? (
          <h4>
            You have proposed <span className="name">{itemOwner}</span> for
            trading <Link to={`/item/${itemId}`}>{itemName}</Link>.
          </h4>
        ) : (
          <h4>
            Your proposal to <span className="name">{itemOwner}</span> for
            trading <Link to={`/item/${itemId}`}>{itemName}</Link> has been
            accepted.
          </h4>
        )}
      </div>
      <div className="tradeBtnWrapper lower">
        {reqStatus === "PENDING" ? (
          ...
          <div className="contactInfoOwner">
            <h4>
              <span className="underline">Owner Contact Info</span>
            </h4>
            {contactInfo}
          </div>
        )}
