  ...
  return (
    <div className="trWrapper" ref={(node) => (wrapper = node)}>
      <div className="upper">
        ...
        <h4>
          <span className="name">{reqMaker}</span> wants to trade with your
          item- <Link to={`/item/${itemId}`}>{itemName}</Link>
        </h4>
      </div>
      <div className="tradeBtnWrapper lower">
        {reqStatus === "PENDING" ? (
          ...
          <button className="acceptBtn normalBtn noLinkBtn">Accepted</button>
        )}
      </div>
    </div>
  );
